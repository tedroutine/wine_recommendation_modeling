...
- 그러므로 user based filtering에서 선별된 와인 중 무작위로 sample으로 선정된 와인을 우선적으로 추천하고 고객들의 리뷰가 쌓이면서 와인 데이터가 업데이트 되어 무작위로 선정된 아이템만 추천되는 한계점이 해소될 예정입니다.

## Bammlier는 여러분의 리뷰가 필요해요.
- 감성의 영역인 와인을 추천하기 때문에 여러분이 추천받은 와인이 마음에 드신다면 좋아요 버튼을 눌러주세요. 
- 좋아요를 받을 수록 여러분에게 더 맞는 와인을 추천해드릴게요.
